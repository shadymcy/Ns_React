...
export default function Detail(props) {
  ...
  const handleStar = () => {
    axios
      .patch(`/news/${props.match.params.id}`, { star: newsData.star + 1 })
      .then((res) => {
        if (res.status === 200) {
  ...
  return (
    <div>
      ...
      >
        <Descriptions size="small" column={3}>
          ...
          <Descriptions.Item label="发布时间">
            {newsData.publishTime ? filterTime(newsData.publishTime) : "-"}
          </Descriptions.Item>
          <Descriptions.Item label="区域">{newsData.region}</Descriptions.Item>
          <Descriptions.Item label="访问数量">
            <span style={{ color: "green" }}>{newsData.view}</span>
          </Descriptions.Item>
          <Descriptions.Item label="点赞数量">
            <span style={{ color: "green" }}>{newsData.star}</span>
          </Descriptions.Item>
      ...
      <div
        ...
        style={{ border: "1px solid silver", margin: "25px" }}
      />
    </div>
  );
}
